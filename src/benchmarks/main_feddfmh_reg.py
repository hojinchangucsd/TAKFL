...
def main_feddfmh_reg(args):
    ...
    num_teachers = int(len(num_users))+1
    public_ds = DatasetKD_AE(p_data, [soft_t for _ in range(num_teachers)])
    ...
    #         optimizer = torch.optim.SGD(net_glob.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=0)
            ...
            for _ in range(args.distill_E):
                ...
                for batch_idx, d2 in enumerate(public_dl):
                    ...
                    if cn==0:
                        self_avg_logits = teacher_logits[2].to(args.device)
                    elif cn==1:
                        self_avg_logits = teacher_logits[3].to(args.device)

                    logits_student = net_glob[cn](teacher_x)
                    
                    kd_loss=0
                    for jj in range(len(num_users)):
                        kd_loss += (1/len(num_users))*kl_criterion(F.log_softmax(logits_student/T, dim=1), 
                                                    F.softmax(teacher_logits[jj]/T, dim=1))
                        
                    self_kd_loss = kl_criterion(F.log_softmax(logits_student/T_self, dim=1), 
                                                F.softmax(self_avg_logits/T_self, dim=1))
                    #kd_loss = mse_criterion(F.softmax(logits_student/T, dim=1), F.softmax(teacher_logits/T, dim=1))/2
                    loss = (T**2) * kd_loss + A_self*(T_self**2)*self_kd_loss
...
def run_feddfmh_reg(args, fname):
    alg_name = 'FedDFMH-Reg'
    ...
    for trial in range(args.ntrials):
        ...
        duration = main_feddfmh_reg(args)

...
import torch.cuda
#from tqdm.contrib.logging import TqdmToLogger
...
def perform_global_inference(model, dataset, args, stream):
    with torch.cuda.stream(stream):
        self_logits = inference(dataset, model, args)
        torch.cuda.synchronize()  # Ensure completion within the stream
    return self_logits

def inference_for_client(cn):
    return inference(public_ds, net_glob[cn], args)

def server_train_model(cn, w_glob, public_ds, args, device):
    # Load the model with global weights
    net = copy.deepcopy(net_glob[cn])
    net.load_state_dict(copy.deepcopy(w_glob[cn]))
    net.to(device)

    public_dl = torch.utils.data.DataLoader(public_ds, batch_size=128, shuffle=True, drop_last=False)
    
    optimizer = torch.optim.Adam(net.parameters(), lr=args.distill_lr, weight_decay=args.distill_wd)
    kl_criterion = nn.KLDivLoss(reduction="batchmean")
    T = args.distill_T
    T_self = 20
    A_self = args.a  # Assuming args.a is defined
    ...
    for idxx in range(3):
        ...
        for _ in range(args.distill_E):
            for batch_idx, d2 in enumerate(public_dl):
                ...
                teacher_x, teacher_y, teacher_logits, self_logits = d2
                teacher_x = teacher_x.to(device)
                teacher_logits = [tl.to(device) for tl in teacher_logits]
                self_logit = self_logits[cn].to(device)

                logits_student = net(teacher_x)

                self_kd_loss = kl_criterion(F.log_softmax(logits_student / T_self, dim=1),
                                            F.softmax(self_logit / T_self, dim=1))

                logit_t = teacher_logits[idxx]
                loss_s = kl_criterion(F.log_softmax(logits_student / T, dim=1), F.softmax(logit_t / T, dim=1))

                loss = (T**2) * loss_s + A_self * (T_self**2) * self_kd_loss

                optimizer.zero_grad()
                loss.backward()
                optimizer.step()

        task_vectors.append(copy.deepcopy(net.state_dict()))
    return task_vectors

def main_fedmhT(args):
    ...
    #archs = ['squeezenet1_0', 'densenet121', 'efficientnet-b3']
    ...
    #     num_teachers = int(len(num_users))
    #     public_ds = DatasetKD_Self(public_train_ds, [soft_t for _ in range(num_teachers)], [soft_t for _ in range(num_teachers)])
    # else:
    #     p_data = torch.utils.data.ConcatDataset([public_train_ds, public_test_ds])
    #     soft_t = np.random.randn(len(p_data), 10)
    #     num_teachers = int(len(num_users))
    #     public_ds = DatasetKD_Self(p_data, [soft_t for _ in range(num_teachers)], [soft_t for _ in range(num_teachers)])
    ...
    for cn in range(len(num_users)):
        ...
        for idx in range(num_users[cn]):
            ...
            clients_tmp.append(Client_FedMH(idx, copy.deepcopy(users_model[cn][idx]), args.local_bs, args.local_ep, optim,
    ...
    for iteration in range(args.rounds):
        ...
        #     # Compute uncertainty weighted average (UWA)
        #     #variances = [1/np.var(mat, axis=1) for mat in logits_locals]
        #     #denominator = sum(variances)
        #     #weighted_average = sum((var / denominator)[:, np.newaxis] * mat for var, mat in zip(variances, logits_locals))
        #     #avg_logits.append(weighted_average)
        #     avg_logits.append(np.mean(logits_locals, axis=0))
            
        # #teacher_logits = np.mean(logits_locals, axis=0)
        # public_ds.set_logits(avg_logits)
        
        # self_logits_list = []
        # for cn in range(len(num_users)):
        #     #self_logits = inference(public_ds, net_glob[cn], args)
        ...
        self_logits_list = []
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(max_len, os.cpu_count() - 1)) as executor:
            # Process each client group (cn)
            for cn in range(len(num_users)):
                jobs = []
                for idx in idxs_users[cn]:
                    stream = streams[idx % max_len]  # Select stream
                    jobs.append(executor.submit(perform_inference, clients[cn][idx], public_ds, stream))
                
                # Wait for all jobs within the group to complete
                concurrent.futures.wait(jobs)
                
                # Gather logits for each client group
                logits_locals = [torch.tensor(job.result()) for job in jobs]
                avg_logits_list.append(torch.mean(torch.stack(logits_locals), dim=0))

                # Parallel computation for self_logits using the global model
                stream = streams[cn % max_len]  # Reuse stream for self logits
                job_self_logits = executor.submit(perform_global_inference, net_glob[cn], public_ds, args, stream)
                concurrent.futures.wait([job_self_logits])
                
                self_logits = job_self_logits.result()
                self_logits_list.append(self_logits)

        for stream in streams:
            stream.synchronize()
        # Assuming public_ds can accept lists of tensors directly. If not, you might need to adjust the code accordingly.
            ...
            for idxx in range(3):
                ...
                for _ in range(args.distill_E):
                    ...
                    for batch_idx, d2 in enumerate(public_dl):
                        ...
                        teacher_logits = [teacher_logits[i].to(args.device) for i in range(len(teacher_logits))]
                        ...
                        loss = (T**2) * loss_s + A_self[cn]*(T_self**2)*self_kd_loss
                        
                        optimizer.zero_grad()
                        loss.backward(retain_graph=True)
                        optimizer.step()
                    #scheduler.step()
                task_vectors.append(copy.deepcopy(net_glob[cn].state_dict()))
                e_time = time.time()
                print(f'Inference & Logit Avg done: {e_time-s_time:.2f}')

            for www in task_vectors:
                for key in w_glob[cn].keys():
                    www[key] = www[key].type_as(w_glob[cn][key])
                    www[key] -= w_glob[cn][key]

            for key in w_glob[cn].keys():
                wl = [0.25, 0.25, 0.5]
                sum_www = sum([wl[jj]*task_vectors[jj][key] for jj in range(len(num_users))])
                sum_www = sum_www.type_as(w_glob[cn][key])
                w_glob[cn][key] += sum_www

...
def main_fedmhw_reg(args):
    ...
    transform_train_pd = transforms.Compose([
    ...
    transform_test_pd = transforms.Compose([
        transforms.Resize((32, 32)),
    ...
    public_train_ds = datasets.ImageNet(root=args.datadir+'imagenet/', split='train', transform=transform_train_pd)
    ...
    train_ds_global = datasets.CIFAR100(root=args.datadir, train=True, transform=transform_train, download=False)
    test_ds_global = datasets.CIFAR100(root=args.datadir, train=False, transform=transform_test, download=False)
    ...
    for k in range(len(num_users)):
        ...
        partitions_test_stat_tmp= dir_partition(num_users[k], niid_beta=args.niid_beta, nclasses=10, 
                                                y_train=Y_train_temp, y_test=Y_test)
    ...
    for iteration in range(args.rounds):
        ...
        for cn in range(len(num_users)):
            ...
            #scale = F.softmax(scale/3, dim=-1)
            scale_list.append(scale)
        print(f'Scale: {scale_list}')
        ##### Global Model KD #####
        for cn in range(len(num_users)):
    ...
    #         optimizer = torch.optim.SGD(net_glob.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=0)
            ...
            for _ in range(args.distill_E):
                ...
                for batch_idx, d2 in enumerate(public_dl):
                    ...
                    self_logits = self_logits[cn].to(args.device)
                    ...
                    for logit_t in teacher_logits:
                        ...
                        #loss_s.requires_grad = True
                        loss_div_list.append(loss_s)
                    
                    loss_div_tensor = torch.Tensor(loss_div_list)
                    if torch.cuda.is_available():
                        scale1 = scale_list[cn].to(args.device)
                        loss_div_tensor = loss_div_tensor.to(args.device).requires_grad_()
                    
                    kd_loss = 0
                    for jj in range(len(num_users)):
                        kd_loss += scale1[jj]*loss_div_list[jj].to(args.device)
                        
                    self_kd_loss = kl_criterion(F.log_softmax(logits_student/T_self, dim=1), 
                                                F.softmax(self_logits/T_self, dim=1))
                    
                    #kd_loss = mse_criterion(F.softmax(logits_student/T, dim=1), F.softmax(teacher_logits/T, dim=1))/2
                    loss = (T**2) * kd_loss + A_self[cn]*(T_self**2)*self_kd_loss

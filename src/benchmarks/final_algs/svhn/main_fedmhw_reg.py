...
def main_fedmhw_reg(args):
    ...
    train_ds_global = datasets.SVHN(root=args.datadir, split='extra', transform=transform_train, download=True)
    test_ds_global = datasets.SVHN(root=args.datadir, split='test', transform=transform_test, download=True)
    ...
    Y_train = train_ds_global.labels

    X_test = test_ds_global.data
    Y_test = test_ds_global.labels
    ...
    for k in range(len(num_users)):
        nsamples = int(1e5)
        inds_subset = indices[k*nsamples:(k+1)*nsamples]

...
def main_fedmhw_reg_scalability(n_clusters, args):
   ...
   # net = ResNet(BasicBlock, config2, num_classes=10)
    ...
    for num, arch in zip(num_users, archs):
        users_model_tmp, net_glob_tmp, initial_state_dict_tmp = get_models_scalability(num_users=num, model='resnet', 
    ...
    for iteration in range(args.rounds):
        ...
        for cn in range(len(num_users)):
            #scale = [num_params_list[i]/num_params_list[cn] for i in range(len(num_users))]
            scale = [num_params_list[i] for i in range(len(num_users))]
            scale = torch.Tensor(scale).float()
            scale = F.softmax(scale/7, dim=-1)
    ...
    #         optimizer = torch.optim.SGD(net_glob.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=0)
            optimizer = torch.optim.Adam(net_glob[cn].parameters(), lr=args.distill_lr, weight_decay=1e-5)
...
def run_fedmhw_reg_scalability(n_clusters, args, fname):
    alg_name = 'FedMHW-Reg-Scalability'
    ...
    for trial in range(args.ntrials):
        ...
        duration = main_fedmhw_reg_scalability(n_clusters, args)

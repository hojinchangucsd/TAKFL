    ...
    for i in range(-1, args.num_users):
        if args.model == "resnet18":
            if args.dataset in ("cifar10"):
                net = ResNet(BasicBlock, [2,2,2,2], num_classes=10).to(args.device)
        elif args.model == "resnet13":
            if args.dataset in ("cifar10"):
                net = ResNet_S(BasicBlock, [2,2], num_classes=10).to(args.device)
        else:
...
def main_fedkd(args):
    ...
    _ = get_dataset_global('cifar10', args.datadir, batch_size=128,
                                        p_train=1.0, p_test=1.0)
    
    soft_t = np.random.randn(50000, 10)
    ...
    users_model, net_glob, initial_state_dict = get_models_kd(args)
    ...
    for idx in range(args.num_users):
        ...
        clients.append(Client_FedKD2(idx, copy.deepcopy(users_model[idx]), args.local_bs, args.local_ep,
    ...
    for idx in range(num_users_FL):
        clients[idx].local_ep=20
        loss = clients[idx].train(public_ds, is_kd=False, is_print=False)
...
#                 clients[jj].lr= 0.001
        ...
        for idx in idxs_users:
            ...
            loss = clients[idx].train(public_ds, is_kd=True, is_print=False)
        ...
        template = "-- Avg Local Acc: {:.3f}"
        print(template.format(np.mean(local_acc)))
    ...
    local_acc = []
    for idx in range(num_users_FL):
        #clients[idx].set_state_dict(copy.deepcopy(w_glob), w_glob_keys)
        loss = clients[idx].train(public_ds, is_kd=True, is_print=False)
...
def run_fedkd(args, fname):
    ...
    for trial in range(args.ntrials):
        ...
        fedavg_ft_local, duration, f1, f2, f3, f4, f5, f6, ff1, ff2, ff3, ff4, ff5, ff6 = main_fedkd(args)

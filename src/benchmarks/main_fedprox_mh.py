...
def main_fedprox_mh(args):
    ...
    for cn in range(len(num_users)):
        ...
        for idx in range(num_users[cn]):
            ...
            mu = 0.001
            clients_tmp.append(Client_FedProx_MH(idx, copy.deepcopy(users_model[cn][idx]), args.local_bs, 
                                args.local_ep, mu, optim, args.lr[cn], args.momentum, args.local_wd[cn], args.lr_scheduler[cn], 
                                args.device, train_dl_local, test_dl_local))

...
def main_fedmhae(args):
    ...
    num_teachers = int(np.dot(fracs, num_users))
    ...
    for iteration in range(args.rounds):
        ...
        public_ds.set_logits(logits_locals)
        ###### Logits Avg #######
        
        ##### Global Model KD #####
        C=0.7
        nu = 1/(C * len(logits_locals))
        for cn in range(len(num_users)):
    ...
    #         optimizer = torch.optim.SGD(net_glob.parameters(), lr=args.lr, momentum=args.momentum, weight_decay=0)
            ...
            for _ in range(args.distill_E):
                ...
                for batch_idx, d2 in enumerate(public_dl):
                    ...
                    loss_div_list = []
                    grads = []
                    logits_student.register_hook(lambda grad: grads.append(
                                                 Variable(grad.data.clone(), requires_grad=False)))
                    
                    for logit_t in teacher_logits:
                        optimizer.zero_grad()
                        #logit_t2 = torch.Tensor(logit_t).to(args.device)
                    ...
                    if batch_idx%50 ==0:
                        print(f'{cn} -- Avg KD Weights: {scale}')

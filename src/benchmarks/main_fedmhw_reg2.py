...
def main_fedmhw_reg(args):
    ...
    print(f'Task Weights: {args.adaptive_weight_T}')
    ...
    #print(f'MODEL: {args.models}, Dataset: {args.dataset}')
        ...
        for cn in range(len(num_users)):
            ...
            for _ in range(args.distill_E):
                ...
                for batch_idx, d2 in enumerate(public_dl):
                    ...
                    loss_weights = torch.tensor(scale_list[cn], dtype=torch.float, device=args.device)
                    kd_loss_list = []
                    for logit_t in teacher_logits:
                        loss_s = kl_criterion(F.log_softmax(logits_student / T, dim=1), 
                                            F.softmax(logit_t / T, dim=1))
                        kd_loss_list.append(loss_s)
                    
                    kd_losses = torch.stack(kd_loss_list)
                    weighted_kd_loss = torch.sum(loss_weights * kd_losses)
                    
                    self_kd_loss = kl_criterion(F.log_softmax(logits_student / T_self, dim=1), 
                            F.softmax(self_logits / T_self, dim=1))
                    
                    loss = (T ** 2) * weighted_kd_loss + A_self[cn] * (T_self ** 2) * self_kd_loss
                    optimizer.zero_grad()

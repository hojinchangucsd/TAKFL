...
def main_fedmhR(args):
    ...
    #     public_ds = DatasetKD_Self2(public_train_ds, soft_t, [soft_t for _ in range(num_clusters)])
    # else:
    #     p_data = torch.utils.data.ConcatDataset([public_train_ds, public_test_ds])
    #     soft_t = np.random.randn(len(p_data), 10)
    #     public_ds = DatasetKD_Self2(p_data, soft_t, [soft_t for _ in range(num_clusters)])
    ...
    for iteration in range(args.rounds):
        ...
        #     avg_logits_list.append(teacher_logits)
